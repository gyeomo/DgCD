...
def get_backbone(name, preserve_readout, pretrained, ratio, rho):
    if not pretrained:
        assert name in ["resnet50", "swag_regnety_16gf"], "Only RN50/RegNet supports non-pretrained network"

    if name == "resnet18":
        network = resnet18(ratio=ratio, rho=rho)#torchvision.models.resnet18(pretrained=True)
        n_outputs = 512
    elif name == "resnet50":
        network = resnet50(ratio=ratio, rho=rho)#
        n_outputs = 2048
