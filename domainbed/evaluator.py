...
def accuracy_from_loader(algorithm, loader, weights,is_test, debug=False):
    correct = 0
    total = 0
    losssum = 0.0
    weights_offset = 0
    ...
    acc = correct / total
    loss = losssum / total
    return acc, loss, latents


def accuracy(algorithm, loader_kwargs, weights, is_test, **kwargs):
    ...
    return accuracy_from_loader(algorithm, loader, weights,is_test, **kwargs)
...
class Evaluator:
    ...
    def evaluate(self, algorithm, args=None, ret_losses=False):
        ...
        for name, loader_kwargs, weights in self.eval_meta:
            ...
            flag = 0
            if is_test and inout=="in":
                flag=1
            elif is_test and inout=="out":
                flag=-1
                
            acc, loss, latent = accuracy(algorithm, loader_kwargs, weights, flag, debug=self.debug)
            accuracies[name] = acc
            losses[name] = loss
